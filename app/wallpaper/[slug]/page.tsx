...
export async function generateMetadata({ params }: WallpaperPageProps, parent: ResolvingMetadata): Promise<Metadata> {
  const { slug } = await params;
  const supabase = await createClient();

  const { data: wallpaper } = await supabase
    .from("wallpapers")
    .select("title, description, image_url, preview_url, category, tags")
    .eq("slug", slug)
    .single();
...
export default async function WallpaperPage({ params }: WallpaperPageProps) {
  ...
  const { data: related } = await supabase
    .from("wallpapers")
    .select("id, slug, title, image_url, preview_url")
  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white selection:bg-red-500/30 pb-24 relative overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <Image src={microscopicBlurUrl} alt="Ambient Glow" fill className="object-cover blur-[120px] opacity-20 scale-125 saturate-150" />
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#09090b]/80 backdrop-blur-2xl border-b border-white/5 shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        <div className="max-w-[1800px] mx-auto px-6 h-20 flex items-center">
          <Link href="/" className="flex items-center gap-3 text-xs font-black uppercase tracking-[0.2em] text-zinc-500 hover:text-white transition-all group active:scale-95">
              ...
              {related.map((wp) => (
                <Link key={wp.id} href={`/wallpaper/${wp.slug}`} className="group space-y-4 block">
                  <div className="relative aspect-[4/5] rounded-[2rem] overflow-hidden bg-zinc-900 border border-white/10 shadow-xl group-hover:border-white/20 transition-colors group-hover:shadow-[0_0_30px_rgba(220,38,38,0.15)]">
                    {/* 🚨 LEAK PLUGGED: Uses preview_url for related images */}
                    <Image 
                      src={wp.preview_url || `${wp.image_url}?width=500&quality=70&format=webp`}
                      alt={wp.title} 
                      fill 
                      unoptimized 
