...
export interface WallpaperCardProps {
  ...
  slug: string;
  src: string;
  previewUrl?: string; // 🚨 Optimized WebP
  vaultKey?: string;   // 🚨 Secure Download Key
  title: string;
  author: string;
  likes: number;
  resolution: string;
...
function WallpaperCard({
  id, slug, src, previewUrl, vaultKey, title, author, likes, resolution,
  width = 800, height = 1000, alt, premium = false, watermark = false, price = null,
}: WallpaperCardProps) {
  ...
  useEffect(() => {
    ...
    if (likedWallpapers.includes(id)) setIsLiked(true);
  }, [id]);
  ...
  return (
    <>
      <Link href={`/wallpaper/${slug}`} prefetch={false} className="group relative block break-inside-avoid rounded-2xl overflow-hidden bg-zinc-900 shadow-sm hover:shadow-xl transition-all duration-300">
        
        {/* 🚨 LEAK PLUGGED: Prioritize previewUrl over heavy src */}
        <Image
          ...
          className={`transition-all duration-500 ${isLoaded ? "opacity-100 scale-100" : "opacity-0 scale-[1.02]"}`}
        />
        ...
        <div className="absolute top-3 left-3 right-3 z-20 flex items-start justify-between pointer-events-none">
          ...
          <button className={`pointer-events-auto opacity-0 group-hover:opacity-100 transition-all duration-300 p-2 rounded-full backdrop-blur-md ${isLiked ? "bg-pink-500/20 text-pink-400 border border-pink-500/30" : "bg-black/40 text-white"}`} onClick={handleLikeToggle}>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 flex items-end justify-between opacity-0 group-hover:opacity-100 translate-y-3 group-hover:translate-y-0 transition-all duration-300 z-20">
          <div>
            <h3 className="text-white text-base sm:text-lg font-semibold leading-tight drop-shadow-md line-clamp-1">{title}</h3>
            <p className="text-zinc-300 text-xs mt-1">{author} • {currentLikes} likes</p>
          </div>

          {/* 🚨 UI UPGRADE: Gold Premium Button Logic */}
          <button
            className={`p-2.5 rounded-full hover:scale-110 active:scale-95 transition-all pointer-events-auto ${
              premium 
                ? "bg-gradient-to-r from-yellow-400 via-amber-500 to-yellow-600 text-black shadow-[0_0_15px_rgba(245,158,11,0.5)] border border-yellow-300/50" 
      ...
      <LoginModal
        ...
        onLogin={() => {
          ...
          setTimeout(() => {
            if (Boolean(premium)) router.push(`/wallpaper/${slug}`);
            else triggerBinaryDownload();
          }, 300);
        }}
      />
    </>
