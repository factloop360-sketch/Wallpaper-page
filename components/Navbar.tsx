...
import LoginModal from "./LoginModal"; // Import your smart modal
      ...
      <div className="sticky top-0 z-50 flex flex-col bg-white/70 dark:bg-[#09090b]/70 backdrop-blur-2xl border-b border-zinc-200/60 dark:border-white/[0.08] transition-colors duration-300">
        ...
        <header className={`${CONTAINER_MAX_WIDTH} w-full ${CONTAINER_PADDING} h-16 sm:h-20 flex items-center justify-between gap-4 lg:gap-8`}>
          ...
          <div className="flex-1 flex items-center justify-end gap-1 sm:gap-3 shrink-0">
            ...
            {/* Desktop Auth Buttons: ADDED onClick logic */}
            <div className="hidden md:flex items-center gap-3 border-l border-zinc-200 dark:border-zinc-800 pl-4 lg:pl-5">
              <button 
                onClick={() => setIsLoginModalOpen(true)}
                className="text-sm font-medium text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-colors px-3 py-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-lg"
              >
                Log in
              </button>
              <button 
                onClick={() => setIsLoginModalOpen(true)}
                className="text-sm font-medium bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-zinc-800 dark:hover:bg-white hover:scale-[1.02] active:scale-[0.98] transition-all px-4 py-2.5 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 shadow-sm"
              >
                Sign up
              </button>
            </div>

            {/* Mobile User Icon: ADDED onClick logic */}
            <button 
              onClick={() => setIsLoginModalOpen(true)}
              className="md:hidden w-10 h-10 flex items-center justify-center text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-800/50 rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
            >
              <UserIcon />
            </button>
          </div>
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`whitespace-nowrap px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 active:scale-95 ${
                    isActive
